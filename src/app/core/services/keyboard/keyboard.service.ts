...
export class KeyboardService {
  ...
  async enterLetter(letter: string): Promise<void> {
    const { boardState, currentBoard, solution } = await firstValueFrom(
      this.store.select(selectEnterWord)
    );
    const { gameStatus } = boardState;
    if (gameStatus === 'IN_PROGRESS') {
      const key = letter.toUpperCase();
      if (key.match('BACKSPACE|DELETE|ESCAPE')) {
        this.store.dispatch(wordleActions.deleteLetter());
      }
      if (currentBoard.length < solution.length) {
        if (LETTERS.indexOf(key) > -1) {
          this.store.dispatch(wordleActions.chooseLetter({ letter }));
        }
      }
